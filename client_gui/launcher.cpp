...
void run_activator() {
    ...
    // Attempt to locate the script relative to current directory
    // If running from client_gui build folder, script is at ../client/activator.py
    // If running from root, script is at client/activator.py

    std::string script_path = "client/activator.py";
    if (!fs::exists(script_path)) {
        if (fs::exists("../client/activator.py")) {
            // We are likely in client_gui or a build folder
            // Change directory to root so the python script runs in expected context
            chdir("..");
        } else {
             std::cerr << "\n\033[1;31mError: Could not find client/activator.py\033[0m" << std::endl;
